...
public class TestTrafficRecognitionByPortDistinction {
	...
	@Test public void higherPortAsSourceHostTestWithTheSameTimestamp() {
		...
		prtcl.setIn_last_switched(now);
		prtcl.setOut_last_switched(now);
		...
		prtcl2.setIn_last_switched(now);
		prtcl2.setOut_last_switched(now);
	...
	@Test public void noHostExchangeIfTimestampNotTheSame() {
		...
		prtcl.setIn_last_switched(before);
		prtcl.setOut_last_switched(after);
		ProtocolRegisterTrigger trigger = new ProtocolRegisterTrigger(null);
		Pair<Host,Host> src_dst = trigger.defineByPort(prtcl);
