...
import de.e2security.e2netwatch.JsonData;
import de.e2security.e2netwatch.business.dao.ElasticSearchDao;
...
import de.e2security.e2netwatch.model.elasticquery.AggsOrder;
...
import de.e2security.e2netwatch.model.elasticquery.QueryClause;
...
import de.e2security.e2netwatch.model.elasticquery.QueryTerm;
import de.e2security.e2netwatch.model.elasticquery.ScriptClause;
import de.e2security.e2netwatch.model.elasticquery.ScriptDetails;
import de.e2security.e2netwatch.model.elasticquery.ScriptQuery;
...
import de.e2security.e2netwatch.model.elasticquery.utils.QueryTermName;
import de.e2security.e2netwatch.model.elasticquery.utils.SortCardinal;
import de.e2security.e2netwatch.model.hostreport.EventDescription;
import de.e2security.e2netwatch.model.newsticker.NewsTickerQueryResultField;
...
public class ElasticSearchDaoRepository implements ElasticSearchDao {
	...
	public JsonReturnData<JsonData> netflow(String ipAddress) {
		...
		ScriptQuery script = new ScriptQuery(
				new ScriptClause(
						new ScriptDetails("(doc['netflow.tcp_flags'].value & 4) > 0")
				)
		);
	...
	private <T> JsonReturnData<T> _getCMDB(final ElasticQuery query, final Class<T> c) {
		final String index = "e2nw-hostinfo";
	...
	private <T> JsonReturnData<T> _getEvents(final ElasticQuery query, final Class<T> c) {
		final String index = "e2nw-events-*";
		return daoManager._getResults(index, query, c);
	}
	...
	private JsonReturnData<QueryResult<EventDescription>> _getEventDescriptions(final ElasticQuery query) {
		final String index = "e2nw-eventdescription";
	...
	private JsonReturnData<QueryResult<NewsTickerQueryResultField>> _getNewsTicker(final ElasticQuery query) {
		final String index = "e2nw-newsticker-*";
	...
	private <T> JsonReturnData<T> _getNetflow(final ElasticQuery query, final Class<T> c) {
		final String index = "e2nw-netflow-*";
		return daoManager._getResults(index, query, c);
	}
