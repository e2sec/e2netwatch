...
package hr.eito.e2nwkite.business.dao.impl;
...
import hr.eito.e2nwkite.business.dao.DockerDao;
...
public class DockerManager implements DockerDao {
	...
	private void containerLifeCycle(CreateContainerCmd createContainerCommand) throws Exception {
		...
		this.connectContainerToe2nwNetwork(containerId);
	...
	/**
	 * Connecting container to e2nw network
	 * 
	 * @param containerId ID of the container
	 * 
	 * @throws Exception
	 */
	private void connectContainerToe2nwNetwork(final String containerId) throws Exception {
		Network network = this.gete2nwNetwork();
	...
	/**
	 * Getting the e2nw Docker network based on network name.
	 * <p>e2nw network name starts with "e2nw" and ends with "_default"
	 * 
	 * @return e2nw Docker network
	 */
	private Network gete2nwNetwork() {
		List<Network> networks = dockerClient.listNetworksCmd().exec();
        for (Network network : networks) {
        	String networkName = network.getName();
            if (StringUtils.startsWith(networkName, "e2nw") && 
