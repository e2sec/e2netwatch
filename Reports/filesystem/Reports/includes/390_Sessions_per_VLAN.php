...
$data = $response['aggregations']['session']['buckets'];
...
foreach ($data as $key1 => $value1) {
    ...
    foreach ($vlandict as $key2 => $value2) {
        $vlan = $value2['key'];
        $output[$timestamp][$vlan] = $value2['doc_count'];
    }
}


$file = fopen("pages/390_Sessions_per_VLAN.tex", "w");
$txt = '\chapter[Session per VLAN]{\underline{Session per VLAN}}' . PHP_EOL . '\begin{flushleft}'  . PHP_EOL;
//
// Add graph description here
//
$txt .= 'The next figure shows the amount of connecitons per VLAN. If the usage within a VLAN changes in a short period of time, it might be related to compromised hosts or larger changes within a specific network.'  . PHP_EOL . '\end{flushleft}' . PHP_EOL . PHP_EOL;
fwrite($file, $txt);

$txt = '\begin{filecontents}{csv/390_Sessions_per_VLAN.csv}' . PHP_EOL;
$txt .= 'Zeitstempel';
...
'\pgfplotstableread[col sep=comma]{csv/390_Sessions_per_VLAN.csv}\vlandata %' . PHP_EOL . 
...
'       font=\tiny' . PHP_EOL . 
'     },' . PHP_EOL . 
'     ylabel={\small{Number of Sessions}},' . PHP_EOL . 
...
foreach ($vlans as $key => $value) {
  ...
  $txt .= '\addplot [fill=RStack' . $i . '] table[x={Zeitstempel}, y=' . $value . '] from {\vlandata};' . PHP_EOL;
  $i = $i + 1;
}
fwrite($file, $txt);
...
'\caption[\normalsize{Session Distribution over VLANs}]{\small{Session Distribution over VLANs}}' . PHP_EOL . 
'\end{figure}' . PHP_EOL;
fwrite($file, $txt);
